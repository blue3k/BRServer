...
namespace BR.Message
{
 	namespace Login
	{
		...
		public class LoginCmd : MessageBase
		{
			...
			public static MessageID MID = new MessageID(MSGTYPE.CMD, true, true, POLICY.LOGIN, 0, 0);
			...
			{
 				Console.WriteLine("{{0}}:LoginCmd: , GameID:{1}, ID:{2}, Password:{3}",
		...
		public class LoginRes : MessageBase
		{
			...
			public static MessageID MID = new MessageID(MSGTYPE.RES, true, true, POLICY.LOGIN, 0, 0);
			...
			public override void TraceOut(string Prefix)
			{
 				Console.WriteLine("{{0}}:LoginRes: , Result:{1}, GameServerAddr:{2}, AccID:{3}, Ticket:{4}, LoginEntityUID:{5}, NickName:{6}",
												Prefix, Result.ToString(), GameServerAddr.ToString(), AccID.ToString(), Ticket.ToString(), LoginEntityUID.ToString(), NickName); 
			} // public override void TraceOut(string Prefix)
			...
			public override void BuildMessageBuffer(BinaryMemoryWriter binWriter)
			{
 				BuildMessageBuffer( binWriter, ref Header , Result, ref GameServerAddr, AccID, Ticket, LoginEntityUID, NickName );
			} // public override void BuildMessageBuffer(BinaryMemoryWriter binWriter)

			static public void  BuildMessageBuffer( BinaryMemoryWriter binaryWriter, ref MessageHeader header , System.Int32 InResult, ref BR.NetAddress InGameServerAddr, System.UInt64 InAccID, System.UInt64 InTicket, System.UInt64 InLoginEntityUID, string InNickName )
			{
 				var binWriter = (PacketMemoryWriter)binaryWriter;
				int __uiMessageSize = sizeof(UInt64)*2 
					+ 4
		...
		public class LoginByFacebookCmd : MessageBase
		{
			...
			public static MessageID MID = new MessageID(MSGTYPE.CMD, true, true, POLICY.LOGIN, 1, 0);
			...
			public override void TraceOut(string Prefix)
			{
 				Console.WriteLine("{{0}}:LoginByFacebookCmd: , GameID:{1}, UID:{2}, FaceBookName:{3}, EMail:{4}, FacebookToken:{5}",
		...
		public class LoginByFacebookRes : MessageBase
		{
			...
			public static MessageID MID = new MessageID(MSGTYPE.RES, true, true, POLICY.LOGIN, 1, 0);
			...
			public override void TraceOut(string Prefix)
			{
 				Console.WriteLine("{{0}}:LoginByFacebookRes: , Result:{1}, GameServerAddr:{2}, AccID:{3}, Ticket:{4}, LoginEntityUID:{5}, NickName:{6}",
												Prefix, Result.ToString(), GameServerAddr.ToString(), AccID.ToString(), Ticket.ToString(), LoginEntityUID.ToString(), NickName); 
			} // public override void TraceOut(string Prefix)
