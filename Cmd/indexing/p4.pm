...
sub GatherFileInformation {
	...
	if ( ! defined $Server ) {
		...
		$Server = '106.247.237.180:60000';
    }
	::status_message("Server $Server  SourceRoot $SourceRoot");
    ...
    foreach (@Files) {
		...
		#::status_message("Pass 1 $_");
		#::status_message("P4Map $3 : $1:$2.");
		...
		if ( $_ =~ m/^(.*)\#(\d*)\s-\s([A-Z]:.*)/i ) {
			...
			#::status_message("Pass 2 $remote_file, $file_revision, $local_file");
			...
			#::status_message("Add Table local:$local_file, remote: $remote_file");
...
sub GetFileInfo {
	...
	#::status_message("GetFile $file in pdb: $name_in_pdb.");
    ...
    if ( defined $$self{'FILE_LOOKUP_TABLE'}{lc $file} ) {
		...
		#::status_message("Exact match File $file");
        #
		...
		#::status_message("Partial match File $file");
        #
