...
namespace SF {
	namespace Svr {
		...
		Result GamePartyEntity::ForeachPlayerGameServer(std::function<Result(PartyPlayer* pPlayer, NetPolicyGameServer& pPolicy)> func)
		{
			for (auto itPlayer = m_PartyPlayerByUID.begin(); itPlayer != m_PartyPlayerByUID.end(); ++itPlayer)
			{
				...
				NetPolicyGameServer netPolicy(pPartyPlayer->GetRemoteEndpoint());
		...
		Result GamePartyEntity::ForeachPlayerSvrGameParty(std::function<Result(PartyPlayer* pPlayer, NetSvrPolicyGameParty& pPolicy)> func)
		{
			for (auto itPlayer = m_PartyPlayerByUID.begin(); itPlayer != m_PartyPlayerByUID.end(); ++itPlayer)
			{
				PartyPlayer* pPartyPlayer = *itPlayer;
				...
				NetSvrPolicyGameParty netPolicy(pPartyPlayer->GetRemoteEndpoint());
				Result hrRes = func(pPartyPlayer, netPolicy);
				if (!(hrRes))
					return hrRes;
			}
		...
		Result GamePartyEntity::JoinPlayer(PartyPlayer*& pPlayer, bool bIsSilent)
		{
			...
			svrCheck(m_PartyPlayerByUID.insert(pPlayer->GetPlayerID(), pPlayer));

			if (!bIsSilent)
			{
				NetSvrPolicyGameParty pJoindPolicy(pPlayer->GetRemoteEndpoint());

				ForeachPlayerSvrGameParty([&](PartyPlayer* pOtherPlayer, NetSvrPolicyGameParty& pOtherPolicy)->Result {
		...
		{
			ScopeContext hr([this, &pPlayer](Result hr)
				{
					Util::SafeDelete(pPlayer);
			...
			if (!bIsSilent)
			{
				ForeachPlayerSvrGameParty([&](PartyPlayer* pOtherPlayer, NetSvrPolicyGameParty& pPolicy)->Result {
					pPolicy.PlayerLeftS2CEvt(pOtherPlayer->GetRouteContext(GetEntityUID()), pPlayer->GetPlayerID());
					return ResultCode::SUCCESS;
					});
			}

			svrCheck(OnPlayerGetOutOfParty(pPlayer, bIsSilent));
