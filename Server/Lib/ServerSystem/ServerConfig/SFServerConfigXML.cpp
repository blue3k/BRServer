...
namespace SF
{
	...
	Result ServerConfigXML::ParseXMLNetPrivate(xmlNode* pNode, ServerConfig::NetPrivate& privateNet)
	{
		...
		privateNet.IP = String(GetMemoryManager(), IP);
	...
	Result ServerConfigXML::ParseXMLNetPublic(xmlNode* pNode, ServerConfig::NetPublic& publicNet)
	{
		...
		publicNet.IPV4 = String(GetMemoryManager(), IPV4);
		publicNet.IPV6 = String(GetMemoryManager(), IPV6);
		publicNet.ListenIP = String(GetMemoryManager(), ListenIP);
	...
	Result ServerConfigXML::ParseXMLGenericServer(xmlNode* pNode, ServerConfig::GenericServer* pGenericServer)
	{
		...
		const String privateNetString = String(GetMemoryManager(), "NetPrivate");

		pGenericServer->UID = atoi(UID);
		pGenericServer->Name = String(GetMemoryManager(), Name);
	...
	Result ServerConfigXML::ParseXMLDBInstance(xmlNode* pNode, ServerConfig::DBInstance* &pDBInstance)
	{
		...
		pDBInstance = new(GetMemoryManager()) ServerConfig::DBInstance(GetMemoryManager());
		pDBInstance->InstanceName = String(GetMemoryManager(), InstanceName);
		pDBInstance->ConnectionString = String(GetMemoryManager(), ConnectionString);
		pDBInstance->UserID = String(GetMemoryManager(), UserID);
		pDBInstance->Password = String(GetMemoryManager(), Password);
	...
	Result ServerConfigXML::ParseXMLDBCluster(xmlNode* pNode, ServerConfig::DBCluster* &pDBCluster)
	{
		...
		pDBCluster->DBInstanceName = String(GetMemoryManager(), DBInstanceName);
		pDBCluster->DBName = String(GetMemoryManager(), DBName);
	...
	Result ServerConfigXML::ParseXMLModule(xmlNode* pNode, ServerConfig::ServerModule* &pServerModule)
	{
		...
		case "ModRanking"_hash32:
			pServerModule = new(GetMemoryManager()) ServerConfig::ServerModule(GetMemoryManager());
			break;

		case "ModMatching_Game_8"_hash32:
			pServerModule = new(GetMemoryManager()) ServerConfig::ServerModuleMatching_8(GetMemoryManager());
		...
		case "ModMatching_Game_4"_hash32:
			pServerModule = new(GetMemoryManager()) ServerConfig::ServerModuleMatching_4(GetMemoryManager());
		...
		case "ModPurchaseValidateGoogle"_hash32:
			pServerModule = new(GetMemoryManager()) ServerConfig::ServerModuleGooglePurchaseValidate(GetMemoryManager());
			((ServerConfig::ServerModuleGooglePurchaseValidate*)pServerModule)->Account = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"Account"));
			((ServerConfig::ServerModuleGooglePurchaseValidate*)pServerModule)->P12KeyFile = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"P12KeyFile"));
			((ServerConfig::ServerModuleGooglePurchaseValidate*)pServerModule)->AuthScopes = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"AuthScopes"));
			break;
		case "ModPurchaseValidateIOS"_hash32:
			pServerModule = new(GetMemoryManager()) ServerConfig::ServerModuleIOSPurchaseValidate(GetMemoryManager());
			((ServerConfig::ServerModuleIOSPurchaseValidate*)pServerModule)->URL = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"URL"));
			((ServerConfig::ServerModuleIOSPurchaseValidate*)pServerModule)->AltURL = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"AltURL"));
			break;
		case "ModLogin"_hash32:
			pServerModule = new(GetMemoryManager()) ServerConfig::ServerModulePublicService(GetMemoryManager());
		...
		if(pServerModule != nullptr)
			pServerModule->ModuleName = String(GetMemoryManager(), (const char*)pNode->name);
	...
	{
		pModuleServer = new(GetMemoryManager()) ServerConfig::ModuleServer(GetMemoryManager());
		auto result = ParseXMLGenericServer(pNode, pModuleServer);
		assert(result);

		result = ForeachElement(pNode->children, [&](xmlNode* pCurNode)
		{
			...
			{
				if(pServerModule != nullptr)
					pModuleServer->Modules.Append(pServerModule);
			}
	...
	Result ServerConfigXML::ParseXMLServerComponent(xmlNode* pNode, ServerConfig::ServerComponent* &pServerComponent)
	{
		...
		case "ComponentGoogle"_hash32:
			pServerComponent = new(GetMemoryManager()) ServerConfig::ServerComponentGoogle(GetMemoryManager());
			((ServerConfig::ServerComponentGoogle*)pServerComponent)->Account = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"Account"));
			((ServerConfig::ServerComponentGoogle*)pServerComponent)->P12KeyFile = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"P12KeyFile"));
			((ServerConfig::ServerComponentGoogle*)pServerComponent)->AuthScopes = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"AuthScopes"));
			break;
		case "ComponentIOS"_hash32:
			pServerComponent = new(GetMemoryManager()) ServerConfig::ServerComponentIOS(GetMemoryManager());
			((ServerConfig::ServerComponentIOS*)pServerComponent)->URL = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"URL"));
			((ServerConfig::ServerComponentIOS*)pServerComponent)->AltURL = String(GetMemoryManager(), (const char*)xmlGetProp(pNode, (xmlChar*)"AltURL"));
			break;
		case "NetPrivate"_hash32:
	...
	{
		pGameServer = new(GetMemoryManager()) ServerConfig::GameServer(GetMemoryManager());
		auto result = ParseXMLGenericServer(pNode, pGameServer);
		assert(result);

		result = ForeachElement(pNode->children, [&](xmlNode* pChild)
		{
			ServerConfig::ServerComponent* pServerComponent = nullptr;
			switch (Hash32((const char*)pChild->name))
			{
			case "NetPrivate"_hash32:
				// ParseXMLGenericServer should handled this
				break;
			case "NetPublic"_hash32:
				result = ParseXMLNetPublic(pChild, pGameServer->PublicNet);
				break;
			default:
				result = ParseXMLServerComponent(pChild, pServerComponent);
				if (pServerComponent != nullptr)
				{
					assert(pServerComponent->ComponentName != nullptr);
					pGameServer->Components.Append(pServerComponent);
				}
				break;
			}
	...
	{
		pGameInstanceServer = new(GetMemoryManager()) ServerConfig::GameInstanceServer(GetMemoryManager());
	...
	Result ServerConfigXML::ParseXMLGameCluster(xmlNode* pNode, ServerConfig::GameCluster* &pGameCluster)
	{
		...
		pGameCluster->GameClusterID = Enum<GameID>().GetValue((const char*)xmlGetProp(pNode, (xmlChar*)"GameID"));
		...
		result = ForeachElement(pNode->children, [&](xmlNode* pChild)
		{
			...
			case "GameTransactionLogDB"_hash32:
			{
				...
				if (pDBCluster != nullptr)
				{
					...
					pGameCluster->DBClusters.Append(pDBCluster);
				}
			}
				break;
			case "ModuleServer"_hash32:
			{
				...
				if(pModuleServer != nullptr)
					pGameCluster->ModuleServers.Append(pModuleServer);
			}
			break;
			case "Game"_hash32:
			{
		...
		result = ForeachElement(pNode, [&](xmlNode* pCurNode)
		{
			...
			case "RankingDB"_hash32:
			{
				...
				if (result)
				{
					...
					m_Config.GetDBClusters().Append(pDBCluster);
				}
			}
			break;
			case "EntityServer"_hash32:
				// Ignore
				break;
			case "ModuleServer"_hash32:
			{
				...
				m_Config.GetServers().Append(pModuleServer);
			}
			break;
	...
	Result ServerConfigXML::LoadConfig(const char* configFilePath)
	{
		...
		if (!StrUtil::StringCmp((char*)root_element->name, -1, "Servers", -1))
		{
	...
	xmlNode* ServerConfigXML::ToXMLNetPrivate(xmlNode* pParent, const ServerConfig::NetPrivate& privateNet) const
	{
		...
		String localBuffer(GetMemoryManager());
	...
	xmlNode* ServerConfigXML::ToXMLNetPublic(xmlNode* pParent, const ServerConfig::NetPublic& publicNet) const
	{
		...
		String localBufferMaxConnection(GetMemoryManager());
