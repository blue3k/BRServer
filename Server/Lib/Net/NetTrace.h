...
#define netChk(e)				{ do{ Result hRes = e; if( !(hRes) ) TrcErrJmp(net,hRes,hr); } while(0); }
#define netChkErr(ErrCode,exp)	{ do{ Result hRes = exp; if( !(hRes) ) TrcErrJmp(net,ErrCode,hr); } while(0); }
