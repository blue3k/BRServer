...
namespace SF
{
 	namespace Svr
	{
 		class CharacterDataServerService : public ServerServiceBase
		{
			...
			Result AttributeValueSubCmd( const uint64_t &InTransactionID, const PlayerID &InPlayerID, const StringCrc32 &InCharacterName, const StringCrc32 &InAttributeName, const FLOAT &InValue );
			// Cmd: Compare and exchange attribute value
			Result AttributeValueCASCmd( const uint64_t &InTransactionID, const PlayerID &InPlayerID, const StringCrc32 &InCharacterName, const StringCrc32 &InAttributeName, const StringCrc32 &InAttributeType, const uint64_t &InExpected, const uint64_t &InNewValue );
