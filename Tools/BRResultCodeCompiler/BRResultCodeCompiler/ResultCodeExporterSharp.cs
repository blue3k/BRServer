        ...
        void WriteBody(StringBuilder output)
        {
            ...
            foreach (var codeItem in m_Codes.ResultCodeItem)
            {
                ...
                if(m_Codes.UseFacilityName)
                    strDefine = string.Format("\t\tconst int {0}_{1}_{2} = ", ResultCode.ServerityToDefineString(codeValue.Severity), facilityUpr, codeItem.CodeName.ToUpper());
                else
                    strDefine = string.Format("\t\tconst int {0}_{1} = ", ResultCode.ServerityToDefineString(codeValue.Severity), codeItem.CodeName.ToUpper());
